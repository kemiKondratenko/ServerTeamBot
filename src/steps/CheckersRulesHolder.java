...
public class CheckersRulesHolder {
    ...
    public List<Position> possibleHeatPositions(Position position) {
    ...
    public List<Position> possibleSimpleStepPositions(Position position) {
        List<Position> positions = new ArrayList<Position>();
    ...
    private boolean calculateNextField(Field currentField, Check check, Position position) {
        return calculateNextField(currentField, check, position, true);
    }


    public boolean calculateNextField(Field currentField, Check check, Position position, boolean needToStep) {
        boolean result = true;
    ...
    private boolean isPositionValid(Field currentField, final Position position) {
        for(Check check : currentField.getAllChecks()){
            if(check.getPosition().equals(position))
                return false;
        }
        return true;
    }
    ...
    private Check getCheckByPosition(Field currentField, final Position enemyPosition) {
        Check enemy = null;
        for (Check check : currentField.getAllChecks()){
            if(enemyPosition.equals(check.getPosition()))
                enemy = check;
        }
        return enemy;
    }
